...
function normalizeExtraction(raw: any): Omit<ReconResult, 'url' | 'title' | 'tabId' | 'timestamp'> {
  const data = raw ?? {};
  return {
...
export async function reconUrl(
  url: string,
  options: { port?: number; host?: string; waitMs?: number; keepTab?: boolean }
): Promise<ReconResult> {
  ...
  try {
    ...
    await (client.Page as any).navigate({ url: encodeURI(decodeURI(url)) });
    ...
    try { await CDP.Close({ port, host, id: target.id }); } catch {}
    throw error;
  }
}
