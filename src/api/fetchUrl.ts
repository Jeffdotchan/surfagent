 ...
 */
export async function fetchUrl(body: FetchBody): Promise<FetchResult> {
  ...
  const timeoutMs = clamp(body.timeoutMs as number, DEFAULT_TIMEOUT_MS, MAX_TIMEOUT_MS);
  ...
  try {
    ...
    const cap = clamp(body.maxBodyBytes as number, DEFAULT_BODY_CAP_BYTES, MAX_BODY_CAP_BYTES);
