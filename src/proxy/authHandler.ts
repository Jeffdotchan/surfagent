 ...
 */
export async function installProxyAuth(client: any): Promise<void> {
  ...
  try {
    ...
    if (process.env.SURFAGENT_DEBUG === '1') {
      console.error('[surfagent] proxy auth install failed:', (e as Error).message);
    }
  }
}
