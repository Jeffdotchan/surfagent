import { pickSticky, type StickyCred } from './pool.js';

function maskSession(password: string): string {
 ...
 */
export function ensureProxyEnvSet(reason: 'launch' | 'restart'): StickyCred | null {
 ...
 */
export function rotateProxyCred(): { cred: StickyCred; previousTag: string } | null {
  const poolFile = process.env.SURFAGENT_PROXY_POOL_FILE;
  if (!poolFile) return null;
